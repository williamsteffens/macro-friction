...
public:
	...
	TriangleMesh(std::vector<Eigen::Vector3f> const& vertices, 
		std::vector<std::array<unsigned int, 3>> const& faces);

	TriangleMesh(float const* vertices,
	...
	Eigen::Vector3f const& vertex(unsigned int i) const { return m_vertices[i]; }
	Eigen::Vector3f& vertex(unsigned int i) { return m_vertices[i]; }
	...
	// Data getters.
	std::vector<Eigen::Vector3f> const& vertex_data() const { 
		return m_vertices; }
	std::vector<Eigen::Vector3f>& vertex_data() { return m_vertices; }    
	std::vector<std::array<unsigned int, 3>> const& face_data() const { 
		return m_faces; }
	std::vector<std::array<unsigned int, 3>>& face_data() { return m_faces; }

	Eigen::Vector3f computeFaceNormal(unsigned int f) const;
